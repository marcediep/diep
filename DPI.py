...
try:
    ...
    while True :       
        ...
        t_start = t_now.replace(hour=15, minute=29, second=40, microsecond=0)
        t_exit = t_now.replace(hour=15, minute=38, second=0, microsecond=0)
        t_sell = t_now.replace(hour=15, minute=29, second=30, microsecond=0)
        today = datetime.datetime.today().weekday()
        if today == 5 or today == 6 :  # 토요일이나 일요일이면 자동 종료
            send_message("**```ansi\n\u001b==  Today is Holiday :)  ==**")
            break
        if t_start < t_now < t_exit :  # 시작 ~ 종료 : 매수
